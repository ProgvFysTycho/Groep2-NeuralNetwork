...
void network::setBias(vector<vector<float>> NetworkBias){
	...
	for(int i = 0; i < NetworkBias.size(); ++i){ //Iterates over the layers off the network
		layer::setBias(NetworkBias.at(i)); //For every layer, sets the weights using the setWeighs function from the layer class
	}
}
...
void network::loadLayers(string fileName)
{
...
//Author: Tycho=========================================================================
void network::saveLayers(string fileName)
{
    ...
    {
    	copy((*iter).begin(),(*iter).end()-1,ostream_iterator<float>(saveFile, commaDelim));
    	saveFile << (*iter).back();
