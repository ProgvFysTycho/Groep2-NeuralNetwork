...
class network {
	public:
		...
		void loadLayers(const string); 				// Loads weigths and biases from a csv file 
	...
	protected:
		...
		vector<vector<float*>> LayerResult;		// The vector of output vectors of each layer. Contains the output of each neuron in each layer.
